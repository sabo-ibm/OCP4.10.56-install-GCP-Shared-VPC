...
18. Copy the sample-install-config.yaml file into the installation directory.

        cp ~/OCP4.10.3-install-GCP-UPI/sample-install-config.yaml ~/$INSTALL_DIR/install-config.yaml
        
19. Edit the copied install-config.yaml as per your environment. The important changes are:
...
20. Export the GCP application credentials. This is the service account key that will be used for creating the remaining cluster components.
